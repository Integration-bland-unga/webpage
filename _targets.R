...
list(
    ...
    tar_target(dt_vaxbo, load_data_table(file.path("data", "f5_2_vaxbo_gen_orig_kon.csv"),
    ...
    tar_target(dt_famindex, load_data_table(file.path("data", "f5_3_famindex_gen.csv"),
    ...
    tar_target(dt_avg_foreign_friends_by_percentage_gen1, 
                load_data_table(file.path("data", "f5_5_fimm_gen_kon_UBak8_sha.csv"),
                                names = c("gender", "composition", "background", "proportion", "lb", "ub"),
    ...
    tar_target(dt_foreign_friends_age, load_data_table(file.path("data","f5_6_futgr_gen_age.csv"),
                                                        names = c("age", "background", "proportion", "lb", "ub"),
                                                        dictionary_cols = "background",
                                                        factor_cols = "background"), format = "fst_dt"),
    tar_target(p_foreign_friends_age, line_plot(dt_foreign_friends_age[, age := as.integer(substr(age,1,2))], "age", "proportion", group_var = "background") + 
                                        scale_y_continuous(labels = scales::label_percent(accuracy = 1), limits = c(0,0.4)) + 
    ...
    tar_target(p_sports, line_plot(dt_sports[, age := as.integer(substr(age,1,2))], "age", "mean", group_var = "background") + 
                                    scale_y_continuous(labels = c("Aldrig", "Mer sällan", "Varje månad", "Varje vecka"), 
    ...
    tar_target(dt_f5_18, rbind(
                           load_data_table(file.path("data", "skolo.csv"),
                                                        ...
                                                        factor_cols = c("origin", "category"))[, place := "I skolan"],
                           load_data_table(file.path("data", "trafiko.csv"),
                                                        names = c("origin", "category", "mean", "lb", "ub"),
                                                        factor_cols = c("origin", "category"))[, place := "I lokaltrafiken"],
                           load_data_table(file.path("data", "butiko.csv"),
                                                        names = c("origin", "category", "mean", "lb", "ub"),
                                                        factor_cols = c("origin", "category"))[, place := "I butiker, caféer osv."],
                           load_data_table(file.path("data", "skolo.csv"),
                                                        names = c("origin", "category", "mean", "lb", "ub"),
    ...
    tar_target(dt_discrimination_ethn, load_data_table(file.path("data", "f5_20_disc4etn.csv"),
                                                names = c("origin", "frequency", "proportion", "lb", "ub"),
                                                factor_cols = c("origin", "frequency"))[frequency == 1, frequency := "En/flera gånger\ni veckan"][
    ...
    tar_target(dt_f5_21, rbind(
                             load_data_table(file.path("data", "attog_swex.csv"),
                                                ...
                                                factor_cols = c("ethnicity", "attitude"))[, background := "Svensk bakgrund"],
   ...
   tar_target(dt_f6_6, set_level_names(
    ...
    tar_target(p_religiosity_age, line_plot(dt_religiosity_age[, age := as.integer(substr(age,1,2))], "age", "mean", group_var = "background") + 
                                        scale_y_continuous(labels = c("Inte alls viktig", "Inte så viktig", "Ganska viktigt", "Mycket viktigt"), 
                                                            breaks = c(0,1,2,3), limits = c(0,3)) + 
    ...
    tar_target(dt_integration_attitudes_background, load_data_table(file.path("data", "f6_24_atint_gen.csv"),
                                                        names = c("age", "background", "proportion", "lb", "ub"),
                                                        factor_cols = c("background"))[1:3, statement := "imm_keep"][
    ...
    tar_target(dt_integration_attitudes_origin, load_data_table(file.path("data", "f6_24_atint_origin.csv"),
                                                        ...
                                                        factor_cols = "origin")[1:6, statement := "imm_keep"][
    ...
    tar_target(p_political_interest_age, line_plot(dt_political_interest_age[, age := as.integer(substr(age,1,2))], "age", "mean", group_var = "background") + 
                                        scale_y_continuous(labels = c("Väldigt lite", "Lite", "En del"), 
    ...
    tar_target(dt_f8_5, load_data_table(file.path("data","f8_5_alc_age.csv"),
                                                ...
                                                dictionary_cols = c("age"),
                                                factor_cols = "gender"), format = "fst_dt"),
    ...
    tar_target(dt_futhea, load_data_table(file.path("data", "futhea_gen.csv"),
                                                    ...
                                                    dictionary_cols = "background",
                                                    factor_cols = c("response", "background"))[, question := "Ha bra hälsa"], format = "fst_dt"),

    tar_target(dt_futjon, load_data_table(file.path("data", "futjob_gen.csv"),
                                                names = c("background", "response", "proportion", "lb", "ub"),
                                                dictionary_cols = "background",
                                                factor_cols = c("response", "background"))[, question := "Ha ett jobb"], format = "fst_dt"),

    tar_target(dt_futmon, load_data_table(file.path("data", "futmon_gen.csv"),
                                                names = c("background", "response", "proportion", "lb", "ub"),
                                                dictionary_cols = "background",
                                                factor_cols = c("response", "background"))[, question := "Ha mycket pengar"], format = "fst_dt"),

    tar_target(dt_futuni, load_data_table(file.path("data", "futuni_gen.csv"),
                                                names = c("background", "response", "proportion", "lb", "ub"),
                                                dictionary_cols = "background",
                                                factor_cols = c("response", "background"))[, question := "Ha universitetsexamen"], format = "fst_dt"),                                         

    tar_target(dt_f8_7, set_level_names(rbind(dt_futhea, dt_futjon, dt_futmon, dt_futuni), 
                                                "response",
                                                c("Nej", "Vet inte", "Ja"))),
